...
void Chinese_Show_one(u8 x, u8 y,  u8 num, u8 size,u8 mode,u8 y_cir)
{
    u8 temp,t,t1;
    u8 offset_y=0,offset_x=0;
    ...
    for(t=0;t<csize;t++)
    {
        ...
        {		
            ...
            if(temp&0x80) LCD_Fast_DrawPoint(x+offset_x,y+(offset_y+y_cir)%size,POINT_COLOR);
            else LCD_Fast_DrawPoint(x+offset_x,y+(offset_y+y_cir)%size,BACK_COLOR);
            temp<<=1;
            offset_y++;
